...
//! Low-level access to semihosting interfaces for Hexagon.
//!
//! Refs:
//! - <https://github.com/quic/qemu/blob/3eb022b82002e58cf0636b01e4ee2b3a7878d822/target/hexagon/hexswi.c>
...
pub fn sys_clock() -> io::Result<usize> {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_CLOCK, ParamRegR::unsigned(0)) };
...
pub unsafe fn sys_close(fd: RawFd) -> io::Result<()> {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_CLOSE, ParamRegR::block(&block)) };
    if ret.unsigned() == 0 {
        Ok(())
    } else {
...
pub fn sys_elapsed() -> io::Result<u64> {
    ...
    // Using one 64-bit field data block works as both cases well.
    let mut block = 0_u64;
    // |                    | on success      | on failure      |                 |
    // | ------------------ | --------------- | --------------- | --------------- |
    // | RETURN REGISTER    | 0               | -1              |                 |
...
pub fn sys_errno() -> io::RawOsError {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_ERRNO, ParamRegR::unsigned(0)) };
    ret.errno()
}

/// SYS_EXCEPTION
// TODO(semver): change return type to !?
pub fn sys_exception(reason: ExitReason) {
    ...
    unsafe {
        syscall_readonly(OperationNumber::SYS_EXCEPTION, arg);
    }
}
pub(crate) fn exit(code: i32) -> ! {
    ...
    // On 64-bit system, SYS_EXIT_EXTENDED call is identical to the behavior of the mandatory SYS_EXCEPTION.
    #[cfg(target_pointer_width = "64")]
    unsafe {
        let block = [ParamRegR::unsigned(reason as usize), ParamRegR::unsigned(subcode)];
        syscall_noreturn_readonly(OperationNumber::SYS_EXCEPTION, ParamRegR::block(&block))
    }
    #[cfg(target_pointer_width = "32")]
    {
        ...
        // If SYS_EXIT_EXTENDED is not supported, above call doesn't exit program, so call SYS_EXCEPTION.
...
pub fn sys_exit_extended(reason: ExitReason, subcode: usize) {
    ...
    // On 64-bit system, SYS_EXIT_EXTENDED call is identical to the behavior of the mandatory SYS_EXCEPTION.
    #[cfg(target_pointer_width = "64")]
    let number = OperationNumber::SYS_EXCEPTION;
    // |                    | always          |                 |
    // | ------------------ | --------------- | --------------- |
    // | RETURN REGISTER    | ?               |                 |
    // | PARAMETER REGISTER | ?               |                 |
...
pub fn sys_flen(fd: BorrowedFd<'_>) -> io::Result<usize> {
    ...
    // | PARAMETER REGISTER | - (unmentioned) | - (unmentioned) | param_unchanged |
    // | block              | - (unmentioned) | - (unmentioned) | readonly        |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_FLEN, ParamRegR::block(&block)) };
    if ret.signed() == -1 { Err(from_errno()) } else { Ok(ret.unsigned()) }
}
...
pub fn sys_get_cmdline_uninit(buf: &mut [MaybeUninit<u8>]) -> io::Result<&mut [u8]> {
    ...
    let ret = unsafe { syscall(OperationNumber::SYS_GET_CMDLINE, ParamRegW::block(&mut block)) };
    if ret.unsigned() == 0 {
...
pub fn sys_iserror(res: isize) -> bool {
    ...
    // | PARAMETER REGISTER | - (unmentioned) | - (unmentioned) | param_unchanged |
    // | block              | - (unmentioned) | - (unmentioned) | readonly        |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_ISERROR, ParamRegR::block(&block)) };
    ret.unsigned() != 0
}
...
pub fn sys_istty(fd: BorrowedFd<'_>) -> io::Result<bool> {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_ISTTY, ParamRegR::block(&block)) };
    match ret.unsigned() {
        1 => Ok(true),
        0 => Ok(false),
...
pub fn sys_open(path: &CStr, mode: OpenMode) -> io::Result<OwnedFd> {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_OPEN, ParamRegR::block(&block)) };
    match ret.raw_fd() {
        Some(fd) => {
...
pub(crate) fn read_uninit<'a>(
    ...
    let ret = unsafe { syscall(OperationNumber::SYS_READ, ParamRegW::block(&mut block)) };
...
pub fn sys_readc() -> u8 {
    ...
    // | RETURN REGISTER    | updated         |                 |
    // | PARAMETER REGISTER | - (unmentioned) | param_unchanged |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_READC, ParamRegR::unsigned(0)) };
    ret.u8()
}
...
pub fn sys_remove(path: &CStr) -> io::Result<()> {
    ...
    // | PARAMETER REGISTER | - (unmentioned) | - (unmentioned) | param_unchanged |
    // | block / path       | - (unmentioned) | - (unmentioned) | readonly        |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_REMOVE, ParamRegR::block(&block)) };
...
pub fn sys_rename(from: &CStr, to: &CStr) -> io::Result<()> {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_RENAME, ParamRegR::block(&block)) };
    if ret.unsigned() == 0 { Ok(()) } else { Err(from_errno()) }
}
...
pub unsafe fn sys_seek(fd: BorrowedFd<'_>, abs_pos: usize) -> io::Result<()> {
    ...
    // | PARAMETER REGISTER | - (unmentioned) | - (unmentioned) | param_unchanged |
    // | block              | - (unmentioned) | - (unmentioned) | readonly        |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_SEEK, ParamRegR::block(&block)) };
    if ret.unsigned() == 0 {
        Ok(())
    } else {
        debug_assert!(ret.signed().is_negative());
...
pub fn sys_system(cmd: &CStr) -> usize {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_SYSTEM, ParamRegR::block(&block)) };
    ret.unsigned()
}
...
pub fn sys_tickfreq() -> io::Result<usize> {
    ...
    // | RETURN REGISTER    | other           | -1              |                 |
    // | PARAMETER REGISTER | - (unmentioned) | - (unmentioned) | param_unchanged |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_TICKFREQ, ParamRegR::unsigned(0)) };
    if ret.signed() == -1 { Err(from_errno()) } else { Ok(ret.unsigned()) }
}
...
pub fn sys_time() -> io::Result<usize> {
    ...
    // | RETURN REGISTER    | updated         |                 |
    // | PARAMETER REGISTER | - (unmentioned) | param_unchanged |
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_TIME, ParamRegR::unsigned(0)) };
    Ok(ret.unsigned())
}
...
pub fn sys_write_orig(fd: BorrowedFd<'_>, buf: &[u8]) -> io::Result<usize> {
    ...
    let ret = unsafe { syscall_readonly(OperationNumber::SYS_WRITE, ParamRegR::block(&block)) };
...
pub fn sys_writec(character: u8) {
    ...
    unsafe {
        syscall_readonly(OperationNumber::SYS_WRITEC, ParamRegR::ref_(&character));
    }
}

/// SYS_WRITECREG
pub fn sys_writecreg(character: u8) {
    // |                    | always          |                 |
    // | ------------------ | --------------- | --------------- |
    // | RETURN REGISTER    | corrupted       |                 |
    // | PARAMETER REGISTER | - (unmentioned) | param_unchanged |
    // | character          | - (unmentioned) | readonly        |
    unsafe {
        syscall_readonly(OperationNumber::SYS_WRITECREG, ParamRegR::unsigned(character as usize));
    }
}
...
pub fn sys_write0(string: &CStr) {
    ...
    unsafe {
        syscall_readonly(OperationNumber::SYS_WRITE0, ParamRegR::c_str(string));
    }
}
