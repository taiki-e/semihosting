...
for p in $(git ls-files '*.sh' '*.rs' '*.ld'); do
    case "${p}" in
        *.sh) prefix="# " ;;
        *.rs) prefix="// " ;;
        *.ld) prefix="/* " ;;
    ...
    if [[ "$(grep -E -n "${prefix}SPDX-License-Identifier: " "${p}")" == "${line}:${prefix}SPDX-License-Identifier: "* ]]; then
        continue
    fi
    license_check_failed='1'
    echo "license-check: ${p}: not found SPDX-License-Identifier at line ${line}"
done
if [[ -n "${license_check_failed}" ]]; then
    error "license-check failed: please add SPDX-License-Identifier to above files"
fi
...
EOF
        ...
        if ! npx cspell --no-progress --no-summary $(git ls-files); then
            ...
            info "you can run $0 or \`npx cspell --no-progress --no-summary \$(git ls-files)\` to check this locally"
        fi
